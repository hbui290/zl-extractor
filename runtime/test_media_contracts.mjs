...
assert.match(source, /source\/attachments/, "attachments must stay under source");
...
assert.match(source, /source.*raw.*attachments\.csv/, "media metadata must stay under source/raw");
...
assert.match(source, /streamResponseToFile/, "large media must be streamed to a temporary file");
assert.doesNotMatch(source, /response\.arrayBuffer\(\)/, "large media must not be buffered in RAM");
assert.match(source, /sha256File/, "existing media must be hash-checked before reuse");
